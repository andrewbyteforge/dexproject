...
class CircuitBreakerManager:
    ...
    async def _persist_breaker_state(self, breaker: EnhancedCircuitBreaker) -> None:
        """Persist circuit breaker state to database using existing persistence layer."""
        if not self.persistence_enabled:
            return

        try:
            from .persistence import CircuitBreakerPersistence
            ...
            await persistence.save_state(
                ...
                current_timeout_multiplier=getattr(breaker, 'current_timeout_multiplier', 1.0),
                consecutive_timeouts=getattr(breaker, 'consecutive_timeouts', 0),
                user_id=getattr(breaker, 'user_id', None),
                chain_id=getattr(breaker, 'chain_id', None)
            )

            self.logger.debug(f"Persisted state for {breaker.name}: {breaker.state.value}")
        except Exception as e:
            self.logger.error(f"Failed to persist breaker state for {breaker.name}: {e}", exc_info=True)

    async def _load_breaker_state(self, breaker: EnhancedCircuitBreaker) -> None:
        """Load persisted circuit breaker state from database using existing persistence layer."""
        if not self.persistence_enabled:
            return
        ...
        try:
            ...
            state_data = await persistence.load_state(breaker.name)

            if state_data:
                # Restore breaker state from loaded data
                breaker.state = CircuitBreakerState[state_data['state']]
                breaker.failure_count = state_data['failure_count']
                breaker.success_count = state_data['success_count']
                breaker.last_failure_time = state_data.get('last_failure_time')
                breaker.last_success_time = state_data.get('last_success_time')

                # Restore metrics if available - FIXED: Create metrics manually instead of using from_dict()
                metrics_data = state_data.get('metrics_data')
                if metrics_data and isinstance(metrics_data, dict):
                    # Create a new CircuitBreakerMetrics instance from the dictionary data
                    breaker.metrics = CircuitBreakerMetrics(
                        ...
                        state_changes=metrics_data.get('state_changes', 0),
                        times_opened=metrics_data.get('times_opened', 0),
                        times_half_opened=metrics_data.get('times_half_opened', 0),
                        total_open_time_seconds=metrics_data.get('total_open_time_seconds', 0.0),
                        ...
                        error_rate_1min=metrics_data.get('error_rate_1min', 0.0),
                        error_rate_5min=metrics_data.get('error_rate_5min', 0.0),
                        error_rate_15min=metrics_data.get('error_rate_15min', 0.0),
                        recovery_attempts=metrics_data.get('recovery_attempts', 0),
                        successful_recoveries=metrics_data.get('successful_recoveries', 0),
                        failed_recoveries=metrics_data.get('failed_recoveries', 0),
                        avg_recovery_time_seconds=metrics_data.get('avg_recovery_time_seconds', 0.0),
                    )

                self.logger.info(
                    f"Loaded persisted state for {breaker.name}: {breaker.state.value} "
                    f"(failures: {breaker.failure_count})"
                )
            else:
                self.logger.debug(f"No persisted state found for {breaker.name}")
        except Exception as e:
            self.logger.error(f"Failed to load breaker state for {breaker.name}: {e}", exc_info=True)

    async def _log_event_to_database(self, event: CircuitBreakerEvent) -> None:
        """Log event to database for audit trail using existing persistence layer."""
        try:
            from .persistence import CircuitBreakerPersistence
            ...
            # FIXED: Correct parameters for log_event method
            # - Added required breaker_type parameter
            # - Changed old_state to previous_state
            # - Changed metadata to details
            await persistence.log_event(
                breaker_name=event.breaker_name,
                breaker_type=event.breaker_type,  # ✅ Added required parameter
                event_type=event.event_type,
                priority=event.priority,
                previous_state=CircuitBreakerState[event.details['old_state']] if 'old_state' in event.details else None,
                new_state=CircuitBreakerState[event.details['new_state']] if 'new_state' in event.details else None,
            ...
            self.logger.debug(f"Logged event to database: {event.event_type} for {event.breaker_name}")
        except Exception as e:
            self.logger.error(f"Failed to log event to database: {e}", exc_info=True)
