...
def api_delete_session(request: HttpRequest, session_id: str) -> JsonResponse:
    ...
    Permanently removes a session and all associated data from the database.
    ...
    URL: DELETE /paper-trading/api/sessions/<session_id>/delete/
    """
    try:
        ...
        # Prevent deletion of active/running sessions
        if session.status in ['RUNNING', 'PAUSED']:
            logger.warning(f"Attempted to delete active session {session_id}")
            return JsonResponse({
                'success': False,
                'error': 'Cannot delete an active session. Please stop it first.'
            }, status=400)
        ...
        # Delete the session (cascade will delete related trades and positions)
        session.delete()
