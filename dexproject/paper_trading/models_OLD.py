...
class PaperTrade(models.Model):
    ...
    Includes metadata for AI decision context, risk scoring,
    and backtesting compatibility with the Smart/Fast Lane engines.
    """
    ...
    # ✅ Added metadata field for AI / analytics context
    metadata = models.JSONField(
        default=dict,
        blank=True,
        help_text="AI trade context (intel level, confidence, reasoning, etc.)",
    )
    ...
    def save(self, *args, **kwargs):
        """
        Override save to validate and fix decimal values before saving.
        
        Prevents corruption from wei values being stored in USD fields,
        which causes SQLite decimal conversion errors.
        """
        
        # Fix amount_in if it looks like wei (> 10^15)
        if self.amount_in and self.amount_in > Decimal('1000000000000000'):
            # Likely wei value, convert to reasonable token amount
            # For ETH: 1e18 wei = 1 ETH, so 1e15 wei = 0.001 ETH
            self.amount_in = Decimal('0.1')
        
        # Ensure USD amounts are reasonable (not wei values)
        if self.amount_in_usd:
            if self.amount_in_usd > Decimal('1000000'):
                # No single trade should be > $1M in paper trading
                self.amount_in_usd = Decimal('100.00')
            elif self.amount_in_usd < Decimal('0.01'):
                # Minimum trade $0.01
                self.amount_in_usd = Decimal('10.00')
        
        # Fix gas costs - should be in reasonable USD range
        if self.simulated_gas_cost_usd:
            if self.simulated_gas_cost_usd > Decimal('1000'):
                # Gas shouldn't cost more than $1000 per transaction
                self.simulated_gas_cost_usd = Decimal('2.50')
            elif self.simulated_gas_cost_usd < Decimal('0.01'):
                # Minimum gas cost
                self.simulated_gas_cost_usd = Decimal('0.50')
        
        # Fix gas price in gwei - typical range is 10-500 gwei
        if self.simulated_gas_price_gwei:
            if self.simulated_gas_price_gwei > Decimal('10000'):
                # Over 10,000 gwei is unrealistic
                self.simulated_gas_price_gwei = Decimal('30')
            elif self.simulated_gas_price_gwei < Decimal('1'):
                self.simulated_gas_price_gwei = Decimal('20')
        
        # Fix expected/actual amounts if they look like wei
        if self.expected_amount_out and self.expected_amount_out > Decimal('1000000000000000'):
            self.expected_amount_out = Decimal('0.1')
        
        if self.actual_amount_out and self.actual_amount_out > Decimal('1000000000000000'):
            self.actual_amount_out = Decimal('0.1')
        
        # Ensure execution time is reasonable (0-60000ms)
        if self.execution_time_ms:
            if self.execution_time_ms > 60000:
                self.execution_time_ms = 500  # Default to 500ms
            elif self.execution_time_ms < 0:
                self.execution_time_ms = 100
        
        # Ensure slippage is reasonable (0-10%)
        if self.simulated_slippage_percent:
            if self.simulated_slippage_percent > Decimal('10'):
                self.simulated_slippage_percent = Decimal('0.5')
            elif self.simulated_slippage_percent < Decimal('0'):
                self.simulated_slippage_percent = Decimal('0.1')
        
        # Ensure gas used is reasonable (21000-1000000)
        if self.simulated_gas_used:
            if self.simulated_gas_used > 1000000:
                self.simulated_gas_used = 50000  # Typical swap gas
            elif self.simulated_gas_used < 21000:
                self.simulated_gas_used = 21000  # Minimum ETH transfer
    ...
    def calculate_pnl(self) -> Decimal:
        ...
        # For buys, P&L requires position tracking (simplified)
        return Decimal('0')
